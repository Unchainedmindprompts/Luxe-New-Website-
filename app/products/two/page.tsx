...
export default function TwoCollection() {
  return <>
    ...
    <section className="container-luxe py-16 md:py-24" id="collection">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">{ranges.map((r, i) => <article key={r.id} id={r.id} className="scroll-mt-28 border border-warm-gray-200 rounded-2xl bg-warm-white overflow-hidden flex flex-col"><div className="relative aspect-[4/3] shrink-0"><Image src={`/images/two/${r.id}.webp`} alt={`TWO ${r.name} ${r.category.toLowerCase()}`} fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 25vw" className="object-cover" /></div><div className="p-6 flex flex-col flex-1"><p className="text-sm text-warm-gray-700">0{i + 1} / {r.category}</p><h3 className="font-serif text-2xl mt-4 text-charcoal">{r.name}</h3><p className="mt-4 text-warm-gray-700 leading-relaxed flex-1">{r.text}</p>{r.id === "weatherwell-elite" ? <Link href="/products/aluminum-shutters" className="mt-6 font-semibold text-charcoal underline underline-offset-4">Explore Weatherwell Elite →</Link> : <a href={`https://two-usa.com/${r.source}/`} target="_blank" rel="noopener noreferrer" className="mt-6 text-sm text-charcoal underline underline-offset-4">View manufacturer details ↗</a>}</div></article>)}</div>
    </section>
