    ...
    <div className="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-8 items-start">
      <div className="space-y-6">
        {shades.map((shade,index) => <article key={shade.id} onFocusCapture={() => onProductSelect(shade.product ?? 'cellular')} className="bg-white rounded-2xl border border-warm-gray-200 p-5 sm:p-8 shadow-sm">
          ...
          {results[index].cents !== undefined && <p className="border-t border-warm-gray-200 mt-6 pt-4 flex justify-between text-sm"><span>{shade.product === 'faux' ? 'Blind' : 'Shade'} estimate · {shade.quantity} {shade.product === 'faux' ? (Number(shade.quantity)===1?'blind':'blinds') : (Number(shade.quantity)===1?'shade':'shades')}{shade.operation.startsWith('motor') && ' · remotes below'}</span><strong>{money(results[index].cents!)}</strong></p>}
        </article>)}
        <button type="button" disabled={shades.length>=25} onClick={() => { changeShades([...shades,fresh(nextId)]); setNextId(n=>n+1); }} className="w-full rounded-xl border border-dashed border-warm-gray-400 py-4 font-semibold hover:bg-cream disabled:opacity-50">+ Add another window</button>
        {remotes.length > 0 && <section aria-label="Remote controls" className="rounded-2xl border border-gold/60 bg-cream p-5 sm:p-8">
          <h3 className="font-serif text-2xl">Your remote controls</h3>
