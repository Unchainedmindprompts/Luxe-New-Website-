      ...
      <div className="space-y-6">
        {shades.map((shade,index) => <article key={shade.id} className="bg-white rounded-2xl border border-warm-gray-200 p-5 sm:p-8 shadow-sm">
          ...
          <fieldset className="mb-6"><legend className="font-semibold text-sm mb-3">Choose your shade style.</legend><div className="grid sm:grid-cols-2 gap-3">
            {([['cellular','Cellular shades','Soft honeycomb fabric.'],['zebra','Zebra shades','Alternating sheer and fabric bands.']] as const).map(([value,title,detail]) => <label key={value} className={`cursor-pointer rounded-xl border p-4 flex gap-3 ${(shade.product ?? 'cellular')===value ? 'border-gold bg-cream ring-1 ring-gold' : 'border-warm-gray-200'}`}><input type="radio" name={`product-${shade.id}`} checked={(shade.product ?? 'cellular')===value} onChange={() => update(shade.id,{product:value,operation:shade.operation.startsWith('motor')?'motor':'cordless'})} className="accent-charcoal mt-1" /><span><span className="block font-medium text-sm">{title}</span><span className="text-xs text-warm-gray-700 mt-1 block">{detail}</span></span></label>)}
