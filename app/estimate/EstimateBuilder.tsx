...
const fresh = (id: number): Shade => ({ id, room: '', width: '', height: '', quantity: '1', light: 'light', operation: 'cordless' });
...
export default function EstimateBuilder() {
  ...
  return <section className="container-luxe py-12 md:py-16" aria-label="Cellular shade estimate builder">
    <div className="mb-8 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm uppercase tracking-[.14em] text-warm-gray-600 mb-2">Your instant estimate</p><h2 className="font-serif text-3xl">Let’s start with your windows.</h2></div><span className="rounded-full bg-linen px-4 py-2 text-sm">¾-inch smooth cellular shades</span></div>
    <div className="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-8 items-start">
      <div className="space-y-6">
        {shades.map((shade,index) => <article key={shade.id} className="bg-white rounded-2xl border border-warm-gray-200 p-5 sm:p-8 shadow-sm">
          ...
          <fieldset className="mt-7"><legend className="font-semibold text-sm mb-3">How would you like the light to feel?</legend><div className="grid sm:grid-cols-2 gap-3">
            {([['light','Light filtering','Soft daylight with everyday privacy.'],['dark','Room darkening','Blackout fabric for a darker room.']] as const).map(([value,title,detail]) => <label key={value} className={`cursor-pointer rounded-xl border p-4 flex gap-3 ${shade.light===value ? 'border-gold bg-cream ring-1 ring-gold' : 'border-warm-gray-200'}`}><input type="radio" name={`light-${shade.id}`} value={value} checked={shade.light===value} onChange={() => update(shade.id,{light:value})} className="accent-charcoal mt-1" /><span><span className="block font-medium text-sm">{title}</span><span className="text-xs text-warm-gray-700 mt-1 block">{detail}</span></span></label>)}
          </div><p className="text-xs text-warm-gray-600 mt-2">Light can still enter around the edges of room-darkening shades.</p></fieldset>
      ...
      <aside className="lg:sticky lg:top-28 rounded-2xl overflow-hidden border border-warm-gray-200">
        ...
        <div className="bg-cream p-7"><p className="text-sm leading-relaxed text-warm-gray-700">Shade pricing only. Installation, shipping, and sales tax are additional. Your final quote follows a measurement and fabric check.</p>
          {shades.some(s=>s.operation.startsWith('motor')) && <p className="text-sm mt-4 text-warm-gray-700">Includes one remote per motorized shade. We can discuss sharing remotes during your consultation.</p>}
